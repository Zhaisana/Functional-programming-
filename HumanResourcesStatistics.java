...
public final class HumanResourcesStatistics {
	...
	public static List<PayrollEntry> payroll(List<Employee> employees) {
		return employees.stream()
				.map(e -> new PayrollEntry(e, e.getSalary(), e.getBonus()))
	...
	public static Employee employeeWithLongestSeniority(List<Employee> employees){
		return employees.stream()
				.reduce(null, HumanResourcesStatistics::_longestSeniority);
	}

	public static BigDecimal highestSalaryWithoutBonus(List<Employee> employees){
		return employees.stream()
				.map(Employee::getSalary)
				.reduce(BigDecimal.valueOf(0), (maxSalary, salary) -> salary.max(maxSalary));
	}

	public static BigDecimal highestSalaryWithBonus(List<Employee> employees){
		return employees.stream()
				.map((Employee::getSalaryPlusBonus))
				.reduce(BigDecimal.valueOf(0), (maxSalary, salary) -> salary.max(maxSalary));
	}

	public static List<Employee> getEmployeesSurnameStartsWithA(Manager manager){
	...
	public static List<Employee> getEmployeesWith1000PlusEarning(List<Employee> employees){
		return employees.stream()
				.filter(e -> e.getSalaryPlusBonus().compareTo(BigDecimal.valueOf(1000)) > 0)
				.collect(Collectors.toList());
	}

	private static Employee _longestSeniority(Employee employee1, Employee employee2) {
		if(employee1 == null) return employee2;
